...
local scriptVersion = "valentine-r1"

print("--- INICIANDO OSAKA " .. scriptVersion .. " (VALENTINE EVENT) ---")
...
local function targetMatchesKeywordList(target, keywords)
	if not target then
		return false
	end
	...
	for _, text in ipairs({
		...
		safeName(target.Parent),
		safeInstancePath(target.Parent),
	}) do
	...
	if prompt then
		for _, text in ipairs({
			...
			safeText(prompt, "ActionText"),
			safeText(prompt, "ObjectText"),
			safeName(prompt.Parent),
			safeInstancePath(prompt.Parent),
		}) do
			if type(text) == "string" and text ~= "" then
				local lowered = string.lower(text)
				for _, keyword in ipairs(keywords) do
...
local function isValentineHeartTarget(target)
	return targetMatchesKeywordList(target, valentineHeartKeywords)
end
...
local function getTargetPriority(target)
	if isValentineHeartTarget(target) then
		return valentineHeartPriority
	end
...
local function getWorldPositionFromInstance(instance)
	...
	return instance.Parent and getWorldPositionFromInstance(instance.Parent) or nil
end
...
local function containsStPatricKeyword(text)
...
local function scoreStPatricPrompt(prompt)
	...
	if activeBrainrots and prompt:IsDescendantOf(activeBrainrots) then
		return 0
	end
	...
	for _, text in ipairs({
		...
		local matched, keyword = containsStPatricKeyword(text)
		if matched then
			score = score + 2
			if keyword == "submit" or keyword == "deliver" or keyword == "deposit" then
				score = score + 4
			elseif keyword == "basket"
				or keyword == "mail"
				or keyword == "box"
				or keyword == "crate"
				or keyword == "heart"
...
local function findStPatricSubmitPrompt()
	...
	if bestPrompt then
		debugLog("VALENTINE_DROP", "prompt=" .. safeInstancePath(bestPrompt) .. " score=" .. tostring(bestScore))
	else
		debugLog("VALENTINE_DROP", "sin prompt de deposito")
	end
...
local function findStPatricYesButton()
	...
	local choiceRoot = choiceGui and choiceGui:FindFirstChild("Choice", true)
	local choices = choiceRoot and choiceRoot:FindFirstChild("Choices")
	local yesButton = choices and choices:FindFirstChild("Yes")
	if yesButton and yesButton:IsA("GuiButton") and safeGuiVisible(yesButton) then
		return yesButton
	end

	for _, descendant in ipairs(playerGui:GetDescendants()) do
		if descendant:IsA("GuiButton") and safeGuiVisible(descendant) then
			local text = string.lower(safeText(descendant, "Text"))
			if text == "yes" or text == "yes!" or text == "confirm" then
				return descendant
			end
		end
	end
...
local function confirmStPatricDialog(status)
	...
	while os.clock() < deadline do
		local yesButton = findStPatricYesButton()
		if yesButton then
			local ok = activateStPatricYesButton(yesButton)
			debugLog("STP_CONFIRM", "button=" .. safeInstancePath(yesButton) .. " ok=" .. tostring(ok))
			if status then
				status.Text = ok and "VALENTINE: CONFIRMANDO..." or "VALENTINE: YES DETECTADO"
			end
			return ok
		end
		task.wait(0.05)
...
local function collectTargetsFromContainer(container, results, scanStats)
	...
	for _, descendant in ipairs(container:GetDescendants()) do
		if isBrainrotCandidate(descendant) then
			...
			if isValentineHeartTarget(descendant) or filtros[rarityName] then
...
local function refreshTargets(force)
	...
	for _, rarityFolder in ipairs(brainrots:GetChildren()) do
		...
		if not filtros[resolvedFolderName] and folderHadCandidate then
			scanStats.filteredOut = scanStats.filteredOut + 1
...
local function approachStPatricPrompt(promptPos, status, runToken)
	if not promptPos then
		return false
	end

	local reached = ghostTravel(promptPos, nil, false, runToken)
	if runToken ~= nil and not isOperationValid(runToken) then
	...
	local goal = CFrame.new(promptPos.X, finalY, promptPos.Z)
	local settled = moveReturnStage(goal, status, nil, runToken)
	local remaining = getGoalDistance(goal)
	debugLog("STP_POT_APPROACH", string.format("promptY=%.2f finalY=%.2f remaining=%.2f", promptPos.Y, finalY, remaining))
	return reached or settled or remaining <= returnGoalTolerance + 0.5
end

local function finalizeStPatricSubmit(status, carryBefore, reason)
	...
	refreshTargets(true)
	returnLocked = false
	status.Text = "VALENTINE: DEPOSITO OK"
	debugLog("VALENTINE_SUBMIT_OK", "reason=" .. tostring(reason) .. " carry_before=" .. tostring(carryBefore))
	releaseAutopilot("VALENTINE: BUSCANDO CORAZONES...", status)
	isReturning = false
	if mainButton then
		updateButtonState(mainButton)
	end
...
local function submitStPatricLoad(status, runToken)
	if runToken ~= nil and not isOperationValid(runToken) then
		debugLog("VALENTINE_SUBMIT_ABORT", "operacion invalidada antes de entregar")
		return false
	end
	if os.clock() - stPatricLastSubmitAttempt < stPatricSubmitCooldown then
		releaseAutopilot("VALENTINE: ESPERANDO DEPOSITO...", status)
		return false
	end

	local carryCount = getEffectiveCarryCount()
	debugLog("VALENTINE_SUBMIT_START", string.format("carry=%d inv=%d returnLocked=%s", carryCount, invCount, tostring(returnLocked)))
	if carryCount <= 0 then
		returnLocked = false
		isReturning = false
		releaseAutopilot("VALENTINE: SIN CARGA", status)
		return false
	end
	...
	if not engageAutopilot("VALENTINE: LLEVANDO CORAZONES...", status) then
		isReturning = false
		return false
	end
	...
	if not prompt or promptScore <= 0 then
		releaseAutopilot("VALENTINE: DEPOSITO NO ENCONTRADO", status)
		isReturning = false
		return false
	end
	debugLog("VALENTINE_SUBMIT_PROMPT", "path=" .. safeInstancePath(prompt) .. " score=" .. tostring(promptScore))

	local promptPos = getWorldPositionFromInstance(prompt)
	if promptPos and not approachStPatricPrompt(promptPos, status, runToken) then
		releaseAutopilot("VALENTINE: NO SE PUDO LLEGAR AL DEPOSITO", status)
		isReturning = false
		return false
	end
	...
	for attempt = 1, 3 do
		if runToken ~= nil and not isOperationValid(runToken) then
			debugLog("VALENTINE_SUBMIT_ABORT", "operacion invalidada durante entrega")
			isReturning = false
			return false
		end
		...
		debugLog("VALENTINE_SUBMIT_TRIGGER", "prompt=" .. safeInstancePath(prompt) .. " ok=" .. tostring(fireOk) .. (fireErr and (" err=" .. tostring(fireErr)) or "") .. " attempt=" .. tostring(attempt))
		task.wait(0.2)
		local confirmOk = confirmStPatricDialog(status)
		debugLog("VALENTINE_CONFIRM_RESULT", string.format("attempt=%d ok=%s carry=%d inv=%d", attempt, tostring(confirmOk), getEffectiveCarryCount(), invCount))

		local deadline = os.clock() + (confirmOk and 5.5 or 4.0)
		while os.clock() < deadline do
			if runToken ~= nil and not isOperationValid(runToken) then
				debugLog("VALENTINE_SUBMIT_ABORT", "operacion invalidada esperando confirmacion")
				isReturning = false
				return false
			end
			...
			if carryNow <= 0 then
				return finalizeStPatricSubmit(status, carryCount, "carry_zero")
			end
			if confirmOk and (not promptNow or promptScoreNow <= 0) then
				return finalizeStPatricSubmit(status, carryCount, "pot_gone")
			end
			task.wait(0.1)
		end
		if confirmOk then
			debugLog("VALENTINE_CONFIRM_STALE", "yes detectado pero sin evidencia de consumo real")
		end
	end

	debugLog("VALENTINE_SUBMIT_FAIL", "sin confirmacion real de entrega")
	isReturning = false
	releaseAutopilot("VALENTINE: ENTREGA FALLIDA", status)
	return false
end
...
local function stopFarm(reason, btn, status, keepWatching)
	...
	status.Text = reason or (keepWatching and "VALENTINE: BUSCANDO CORAZONES..." or "VALENTINE: ESPERANDO")
	resetRunState()
	updateButtonState(btn)
end
...
btn.MouseButton1Click:Connect(function()
	...
	if watchMode then
		...
		debugLog("VALENTINE_ON", string.format("base=(%.2f, %.2f, %.2f)", root.Position.X, root.Position.Y, root.Position.Z))
		debugLog("MONITOR_SESSION", "watch valentine iniciado")
		basePos = root.Position
		flyValue.Value = root.CFrame
		root.Anchored = false
		captureBaselineTools()
		...
		lastAnchorInterferenceLog = 0
		lastDesyncInterferenceLog = 0
		debugLog("VALENTINE_OFF", "script en espera")
		debugLog("MONITOR_SESSION", "watch detenido")
		releaseAutopilot("VALENTINE: ESPERANDO", status)
		resetRunState()
	end
end)
	...
	while task.wait(0.08) do
		local ok, err = xpcall(function()
			...
			if (returnLocked or carryCount >= returnAt) and not isReturning then
				debugLog("VALENTINE_TRIGGER", "returnLocked=" .. tostring(returnLocked) .. " inv=" .. tostring(invCount) .. " carry=" .. tostring(carryCount))
				returnLocked = true
			...
			if not target then
				...
				if (invCount > 0 or returnLocked) and not isReturning and not isGrabbing then
					...
					currentTarget = nil
					releaseAutopilot(lastScanHint ~= "" and lastScanHint or "VALENTINE: BUSCANDO CORAZONES...", status)
				end
				return
			end
			...
			if success then
				...
				debugLog("VALENTINE_GRAB_OK", "inv=" .. tostring(invCount) .. " total=" .. tostring(sessionGrabCount) .. " tools=" .. tostring(getFarmToolCount()))
				...
				removeFromCache(target)
				refreshTargets(true)
				status.Text = "VALENTINE: " .. tostring(invCount) .. "/" .. tostring(returnAt)

				if returnLocked or invCount >= returnAt then
					debugLog("VALENTINE_TRIGGER", "limite alcanzado")
					submitStPatricLoad(status, runToken)
				else
					task.wait(0.12)
				end
				...
				if returnLocked or carryAfterFail >= returnAt then
					returnLocked = true
					grabAttempts = 0
					debugLog("VALENTINE_TRIGGER", "limite detectado tras grab abort carry=" .. tostring(carryAfterFail) .. "/" .. tostring(returnAt))
					submitStPatricLoad(status, runToken)
					return
				end
				grabAttempts = grabAttempts + 1
				debugLog("VALENTINE_GRAB_FAIL", "fail intento=" .. tostring(grabAttempts))
				status.Text = "VALENTINE FAIL " .. tostring(grabAttempts) .. "/3"
